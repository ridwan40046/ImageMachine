...
extension UIBarButtonItem {
    ...
    convenience init (fontAwesomeTitle: String?, color: UIColor? = nil, target: Any?, action: Selector?) {
        ...
        if let color = color {
            ...
            self.setTitleTextAttributes([NSAttributedStringKey.foregroundColor: color], for: .highlighted);
        }
    }
    
    convenience init (lineAwesomeTitle: String?, color: UIColor? = nil, target: Any?, action: Selector?) {
